...
#define LOG_EVENT "Global\\ChyLogWrite" 
CString CLog::s_strLogFile = "";  
CString CLog::s_strLogPrefix = "";
CString CLog::s_errorInfo= "";
...
short CLog::SetLogFile(LPCTSTR strPath)
{
	...
	{
		s_strLogFile.Format("%s.log",strPath);
	} 
	return 1;
}
...
void CLog::CreateNewFile()
{
	...
	{
		CString str;
		str.Format("打开文件失败！路径为：%s",s_strLogFile);
		AfxMessageBox(str);
		exit(0);
	}
	file.Close();
